...
function crearCliente(path, port){
    ...
    async function obtenerTodos(token){
        const options = {
            method: 'get',
            uri: serverPath + apiPath + '/consultar',
            headers: {
    ...
    async function agregarDireccion(direccion, token){
        const options = {
            method: 'post',
            uri: serverPath + apiPath + '/agregar',
            headers: {
                Authorization: `Bearer ${token}`
            },
            body: direccion,
    ...
    async function modificarDireccion(idUsuario, idDireccion, direccion, token){
        const options = {
            method: 'put',
            uri: serverPath + apiPath + + '/modificar/' + idUsuario + '/' + idDireccion,
            headers: {
                Authorization: `Bearer ${token}`
            },
            body: direccion,
    ...
    async function eliminarDireccion(idUsuario, direccion, token){
        const options = {
            method: 'delete',
            uri: serverPath + apiPath + '/eliminar/' + idUsuario + '/' + direccion,
            headers: {
                Authorization: `Bearer ${token}`
            },
            json: true
