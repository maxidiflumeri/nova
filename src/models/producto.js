...
import mensajes from '../messages/mensajes.js'
...
async function modificarProducto(id, producto) {
    ...
    if (validarProducto(producto)) {
        ...
        if (prod.length > 0) {
            const marca = await models.marca.obtenerPorId(producto.ID_MARCA)
            if (marca.length > 0) {
                const tipo = await models.tipo.obtenerPorId(producto.ID_TIPO)
                if (tipo.length > 0) {
                    try {
